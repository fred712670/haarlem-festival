                        ...
                        <div class="booking-select-wrapper">
                            <select name="date" id="date" required>
                                ...
                                <?php foreach ($availableDates as $date): ?>
                                    <option value="<?= htmlspecialchars($date['TourDate']) ?>">
                                        <?= date('l, F j, Y', strtotime($date['TourDate'])) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="booking-select-wrapper">
                            <select name="time" id="time" required>
                                ...
                                <?php foreach ($availableTimes as $time): ?>
                                    <option value="<?= htmlspecialchars($time['TourTime']) ?>">
                                        <?= date('g:i A', strtotime($time['TourTime'])) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="booking-field">
                        ...
                        <div class="booking-radio-group">
                            <div class="booking-radio">
                                <input type="radio" id="regular" name="ticketType" value="Regular Participant" checked>
                                <input type="hidden" name="price" value="17.50">
                                <label for="regular">Regular Participant (€17.50)</label>
                            </div>
                            <div class="booking-radio">
                                <input type="radio" id="family" name="ticketType" value="Family Package Deal">
